  ...
  const handleAddIssuesToView = async (data: ISearchIssueResponse[]) => {
    ...
    try {
      ...
      setToast({
        ...
        message: moduleId
          ? t("issue.add.module.success", { count: issues.length })
          : t("issue.add.cycle.success", { count: issues.length }),
      });
    } catch (_error) {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("common.error.label"),
        message: moduleId ? t("issue.add.module.failed") : t("issue.add.cycle.failed"),
      });
    }
  };
        ...
        {!disableIssueCreation &&
          (renderExistingIssueModal ? (
            ...
            >
              ...
              >
                <span className="flex items-center justify-start gap-2">{t("issue.create.label")}</span>
              </CustomMenu.MenuItem>
              <CustomMenu.MenuItem
                onClick={() => {
