  ...
  const getCycleDetails = async (newCycleId: string) => {
    ...
    await Promise.all(cyclesFetch).catch((error) => {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("error"),
        message: error.error || t("cycle_transfer.toasts.fetch_error"),
      });
    });
  };
      ...
      <div className="flex flex-col gap-4 py-5">
        <div className="flex items-center justify-between px-5">
          <div className="flex items-center gap-1">
            ...
            <h4 className="text-18 font-medium text-primary">{t("cycle_transfer.title")}</h4>
          </div>
        ...
        <div className="flex items-center gap-2 border-b border-subtle px-5 pb-3">
          ...
          <input
            ...
            placeholder={t("cycle_transfer.search_placeholder")}
