...
const errorCodeMessages: {
  [key in EAuthenticationErrorCodes]: { title: string; message: string };
} = {
  // global
  ...
  [EAuthenticationErrorCodes.USER_ACCOUNT_DEACTIVATED]: {
    ...
    message: `User account deactivated. Please contact {contact}.`,
  },
...
const linkedErrorCodes: Partial<
  Record<
    EAuthenticationErrorCodes,
    {
      href: (email?: string) => string;
      prefix: string;
      link: string;
      suffix: string;
    }
...
> = {
  [EAuthenticationErrorCodes.USER_ALREADY_EXIST]: {
    href: (email = undefined) => `/sign-in${email ? `?email=${encodeURIComponent(email)}` : ``}`,
    prefix: "Your account is already registered. ",
    link: "Sign In",
    suffix: " now.",
  },
  [EAuthenticationErrorCodes.USER_DOES_NOT_EXIST]: {
    href: (email = undefined) => `/${email ? `?email=${encodeURIComponent(email)}` : ``}`,
    prefix: "No account found. ",
    link: "Create one",
    suffix: " to get started.",
  },
  [EAuthenticationErrorCodes.ADMIN_USER_ALREADY_EXIST]: {
    href: () => `/admin`,
    prefix: "Admin user already exists. ",
    link: "Sign In",
    suffix: " now.",
  },
  [EAuthenticationErrorCodes.ADMIN_USER_DOES_NOT_EXIST]: {
    href: () => `/admin`,
    prefix: "Admin user does not exist. ",
    link: "Sign In",
    suffix: " now.",
  },
};

const getAuthErrorText = (t: TTranslate | undefined, key: string, fallback: string, params?: Record<string, unknown>) =>
  t ? t(key, params) : fallback;

const getSimpleAuthErrorMessage = (
  errorCode: EAuthenticationErrorCodes,
  fallback: string,
  t: TTranslate | undefined
) => {
  if (errorCode === EAuthenticationErrorCodes.USER_ACCOUNT_DEACTIVATED) {
    const contact = SUPPORT_EMAIL || getAuthErrorText(t, "auth.errors.contact.administrator", "administrator");
    return getAuthErrorText(t, `auth.errors.${errorCode}.message`, fallback.replace("{contact}", contact), {
      contact,
    });
  }

  return getAuthErrorText(t, `auth.errors.${errorCode}.message`, fallback);
};

const getLinkedAuthErrorMessage = (
  errorCode: EAuthenticationErrorCodes,
  email: string | undefined,
  t: TTranslate | undefined
) => {
  const linkedErrorCode = linkedErrorCodes[errorCode];
  if (!linkedErrorCode) return undefined;

  return (
    <div>
      {getAuthErrorText(t, `auth.errors.${errorCode}.message.prefix`, linkedErrorCode.prefix)}
      <Link
        className="font-medium underline underline-offset-4 transition-all hover:font-bold"
        href={linkedErrorCode.href(email)}
      >
        {getAuthErrorText(t, `auth.errors.${errorCode}.message.link`, linkedErrorCode.link)}
      </Link>
      {getAuthErrorText(t, `auth.errors.${errorCode}.message.suffix`, linkedErrorCode.suffix)}
    </div>
  );
};
...
export const authErrorHandler = (
  ...
  if (bannerAlertErrorCodes.includes(errorCode))
    return {
      ...
      code: errorCode,
      title: getAuthErrorText(t, `auth.errors.${errorCode}.title`, errorCodeMessages[errorCode]?.title || "Error"),
      message:
        getLinkedAuthErrorMessage(errorCode, email, t) ||
        getSimpleAuthErrorMessage(
          errorCode,
          errorCodeMessages[errorCode]?.message || "Something went wrong. Please try again.",
          t
        ),
    };
